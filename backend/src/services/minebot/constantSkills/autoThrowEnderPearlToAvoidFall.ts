...
class AutoThrowEnderPearlToAvoidFall extends ConstantSkill {
    private holdItem: HoldItem;
    private shootItemToEntityOrBlockOrCoordinate: ShootItemToEntityOrBlockOrCoordinate;
    constructor(bot: CustomBot) {
        super(bot);
        this.skillName = 'auto-throw-ender-pearl-to-avoid-fall';
        this.description = '落下死しそうな時に自動でエンダーパールを投げて落下ダメージを回避します';
        this.interval = 100;
        this.isLocked = false;
        this.status = false;
        this.holdItem = new HoldItem(this.bot);
        this.shootItemToEntityOrBlockOrCoordinate = new ShootItemToEntityOrBlockOrCoordinate(this.bot);
    }
    ...
    async run() {
        ...
        for (let y = Math.floor(pos.y) - 1; y >= 0; y--) {
            ...
            if (block && block.boundingBox !== 'empty' && block.name !== 'air' && block.name !== 'cave_air') {
                groundY = y + 1;
                break;
            }
        }
        ...
        this.shootItemToEntityOrBlockOrCoordinate.shootToCoordinate(targetPos, 'ender_pearl');
...
export default AutoThrowEnderPearlToAvoidFall;
