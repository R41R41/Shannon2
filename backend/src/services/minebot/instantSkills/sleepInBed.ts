...
class SleepInBed extends InstantSkill{
            ...
            for (let z = -searchRadius; z <= searchRadius; z++) {
                for (let y = -1; y <= 1; y++) {
                    ...
                    if (blockPos && blockPosHead && blockBelow && blockBelowHead) {
                        if ((blockPos.name === 'air' || blockPos.name === 'cave_air') && 
                            (blockPosHead.name === 'air' || blockPosHead.name === 'cave_air') && 
                            blockBelow.name !== 'air' && blockBelow.name !== 'cave_air' && 
                            blockBelowHead.name !== 'air' && blockBelowHead.name !== 'cave_air') {
                            return pos;
                        }
                    }
    ...
    async run(){
        try{
            ...
            if (!bedItem) {
                return {"success": false, "result": "インベントリにベッドがなく、周囲にもベッドが見つかりませんでした"};
            }

            // ベッドを設置できる場所を探す
            const placePos = await this.findPlaceForBed();
            if (!placePos) {
                return {"success": false, "result": "ベッドを設置できる適切な場所が見つかりませんでした"};
            }

            // 設置場所に移動
            await this.bot.pathfinder.goto(new goals.GoalNear(placePos.x, placePos.y, placePos.z, 2));
            ...
            if (!referenceBlock) {
                return {"success": false, "result": "ベッドを設置するための参照ブロックが見つかりませんでした"};
            }
