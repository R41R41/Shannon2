...
class SearchAndGotoBlock extends InstantSkill {
  ...
  async runImpl(blockName: string, coordinate: Vec3) {
    ...
    try {
      if (coordinate) {
        const attemptToReachGoal = this.attemptToReachGoal(blockName, coordinate);
      ...
      // 到達を試行する関数
      const attemptToReachGoal = this.attemptToReachGoal(blockName, targetPos);
      // 到達試行を開始
      return await attemptToReachGoal;
    } catch (error: any) {
  ...
  private async attemptToReachGoal(
    blockName: string,
    targetPos: Vec3,
    remainingAttempts = 3,
    ...
    try {
      ...
      if (remainingAttempts > 1) {
        ...
        return this.attemptToReachGoal(blockName, targetPos, remainingAttempts - 1);
      } else {
