...
class BuildArchitecture extends InstantSkill {
    constructor(bot: CustomBot) {
        ...
        this.description = 'placeBlockを複数回使用して指定された設計図を建築します。';
        this.params = [
            {
    ...
    async run(architectureName: string, placePosition: Vec3) {
        try {
            // 設計図のJSONファイルを読み込む
            const architectureData = await this.loadArchitectureData(architectureName);
            if (!architectureData) {
                return {
                    success: false,
                ...
                for (const block of remainingBlocks) {
                    ...
                    if (existingBlock && existingBlock.name !== 'air') {
                        ...
                        if (!existingBlock.name.includes(block.name)) {
                            try {
                                console.log(`異なるブロック "${existingBlock.name}" を破壊します。期待値: "${block.name}"`);
                    ...
                    // インベントリからブロックを検索
                    const itemInInventory = this.bot.inventory.items().find(item => 
                        item.name.includes(block.name)
                    );
                    
                    if (!itemInInventory) {
                        console.log(`ブロック "${block.name}" がインベントリにありません。スキップします。`);
                        newRemainingBlocks.push(block); // 後で再試行
                        continue;
                    }
                    ...
                    if (!referenceBlock) {
                        console.log(`設置する隣接ブロックが見つかりません。足場を試みます。座標: (${blockPos.x}, ${blockPos.y}, ${blockPos.z})`);
                        ...
                        if (await this.placeScaffoldBlock(scaffoldPos)) {
                            console.log(`足場ブロックを設置しました。座標: (${scaffoldPos.x}, ${scaffoldPos.y}, ${scaffoldPos.z})`);
                            // 足場ブロックを記録して後で再試行
                    ...
                    try {
                        ...
                        const result = await placeBlock.run(block.name, blockPos, referenceBlock.position);
                        if (result.success) {
                            console.log(`ブロック "${block.name}" を座標 (${blockPos.x}, ${blockPos.y}, ${blockPos.z}) に設置しました。`);
                            placedBlocks++;
                            madeProgress = true;
                        } else {
            ...
            return {
                success: true,
                result: `設計図 "${architectureName}" の建築を完了しました。全 ${totalBlocks} ブロックを設置しました。`
            };
    ...
    private async placeScaffoldBlock(scaffoldPos: Vec3): Promise<boolean> {
        try {
            ...
            for (const option of scaffoldOptions) {
                scaffoldItem = this.bot.inventory.items().find(item => 
            ...
            if (!scaffoldRefBlock) {
                console.log('足場用ブロックを設置するための隣接ブロックが見つかりません。');
                return false;
            }
            ...
            const result = await placeBlock.run(scaffoldItem.name, scaffoldPos, scaffoldRefBlock.position);
            if (result.success) {
                console.log(`足場ブロック "${scaffoldItem.name}" を座標 (${scaffoldPos.x}, ${scaffoldPos.y}, ${scaffoldPos.z}) に設置しました。`);
                // 少し待機して連続設置によるエラーを防止
                await new Promise(resolve => setTimeout(resolve, 250));
                return true;
            } else {
    ...
    // 設計図データを読み込む
    private async loadArchitectureData(architectureName: string): Promise<Architecture | null> {
        try {
