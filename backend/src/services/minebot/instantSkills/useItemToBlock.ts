...
class UseItemToBlock extends InstantSkill {
  ...
  constructor(bot: CustomBot) {
    ...
    this.description =
      '指定したアイテムを指定したブロックに対して使用します。種を植える際や、原木の皮を剥ぐ際などに使用します。';
    this.priority = 100;
    ...
    this.params = [
      {
        name: 'itemName',
        description: '使用するアイテムの名前。例: iron_axe, wheat_seeds, など',
        type: 'string',
      },
      {
        name: 'targetBlockName',
        description: '使用するブロックの名前。例: oak_log, farmland, など',
        type: 'string',
      },
      {
        name: 'blockCount',
      ...
      },
      {
        ...
        description: '上にブロックがないブロックのみに使用するかどうか',
        type: 'boolean',
      },
    ];
  }
  ...
  async runImpl(
    ...
    try {
      ...
      if (!Block) {
        return {
          success: false,
          result: `ブロック${targetBlockName}はありません`,
        };
      }
      let count = 0;
