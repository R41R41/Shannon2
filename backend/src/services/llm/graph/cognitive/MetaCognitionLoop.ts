...
const SubtaskUpdateSchema = z.object({
    id: z.string().describe('サブタスクID (例: st_1)'),
    status: z.enum(['completed', 'error', 'skipped']).describe('新しいステータス'),
    result: z.string().optional().describe('完了時の結果'),
...
const MetaOutputSchema = z.object({
    ...
    reasoning: z.string()
        .describe('判断の理由（1-2文）'),
    // プラン管理（プランがある場合のみ使用）
    currentSubtaskId: z.string().nullable().optional()
        .describe('現在実行中と判断されるサブタスクID（変更がなければ省略）'),
    subtaskUpdates: z.array(SubtaskUpdateSchema).optional()
        .describe('ステータスが変わったサブタスクの一覧（変更がなければ省略）'),
});
...
- インベントリに必要素材がある場合: 「既にraw_ironを2個所持しているので、新たに採掘せず精錬から始めるべき」

## プラン管理（プランが提示された場合）
- currentSubtaskId: ツール呼び出し履歴から、現在どのサブタスクに取り組んでいるか判断して設定する
- subtaskUpdates: ステータスが変わったサブタスクを報告する
  - 完了したサブタスクは status='completed' にする
  - **サブタスクが5イテレーション以上 in_progress で成功率50%以下 → status='error' にして代替手段を suggestion に書く**
  - 前提条件が変わって不要になったサブタスクは status='skipped' にする
- サブタスクが error になった場合、suggestion で具体的な代替手段を指示する（例: 「移動できないので、crafting_tableをインベントリから取り出して現在地に設置する」）`;

const EVALUATE_INTERVAL_ITERATIONS = 3;
const MIN_EVALUATE_INTERVAL_MS = 5_000;
/** Phase 3-D: 緊急時はメタ認知介入を高速化（-3秒） */
...
export class MetaCognitionLoop {
    ...
    private async evaluate(trigger: string): Promise<void> {
        ...
        try {
            ...
            // インベントリ情報を構築
            const inventoryInfo = this.formatInventoryForEvaluate(snapshot.inventory);

            // プランコンテキスト構築
            const planInfo = this.formatPlanForEvaluate(snapshot.plan);
            ...
            this.applyDecision(metaState, response);
        } catch (error) {
            logger.error('[MetaCognition] evaluate error:', error);
        }
    }

    private applyDecision(meta: MetaState, llmResponse?: z.infer<typeof MetaOutputSchema>): void {
        // モデルエスカレーション/デエスカレーション
        ...
        // フィードバック注入（on_track のときは介入しない — 順調なら黙る）
        if (meta.assessment !== 'on_track' && meta.suggestion && this.feedbackCallback) {
            this.feedbackCallback(`[メタ認知] ${meta.suggestion}`);
        }

        // プラン管理: MetaCognition がサブタスク進捗を更新
        if (llmResponse && this.blackboard.plan) {
            this.applyPlanUpdates(llmResponse);
        }
    ...
    private applyPlanUpdates(response: z.infer<typeof MetaOutputSchema>): void {
        const plan = this.blackboard.plan;
        if (!plan) return;

        // サブタスクステータス更新
        if (response.subtaskUpdates && response.subtaskUpdates.length > 0) {
            for (const update of response.subtaskUpdates) {
                this.blackboard.patchPlanSubtask(update.id, {
                    status: update.status,
                    result: update.result,
                    failureReason: update.failureReason,
                });
                logger.info(
                    `[MetaCognition] 📋 サブタスク更新: ${update.id} → ${update.status}${update.failureReason ? ` (${update.failureReason})` : ''}`,
                );
            }
        }

        // 現在サブタスクの切り替え
        if (response.currentSubtaskId && response.currentSubtaskId !== plan.currentSubtaskId) {
            // 前のサブタスクが in_progress のままなら pending に戻す
            const prev = plan.subtasks.find(
                s => s.id === plan.currentSubtaskId && s.status === 'in_progress',
            );
            if (prev) {
                this.blackboard.patchPlanSubtask(prev.id, { status: 'pending' });
            }
            // 新しいサブタスクを in_progress に
            this.blackboard.patchPlanSubtask(response.currentSubtaskId, { status: 'in_progress' });
            ...
            logger.info(
                `[MetaCognition] 📋 サブタスク切替: ${plan.currentSubtaskId} → ${response.currentSubtaskId}`,
            );
        }
    }
    ...
    private formatInventoryForEvaluate(inventory: import('./CognitiveBlackboard.js').InventoryEntry[] | null): string {
    ...
    private formatPlanForEvaluate(plan: import('./CognitiveBlackboard.js').PlanState | null): string {
        if (!plan || plan.subtasks.length === 0) return '';

        const lines = plan.subtasks.map(st => {
            const statusIcon = { pending: '⬜', in_progress: '🔄', completed: '✅', error: '❌', skipped: '⏭️' }[st.status];
            ...
            return `  ${statusIcon} ${st.id}: ${st.goal}${iterInfo}${extra}`;
        }).join('\n');
