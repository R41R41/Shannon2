...
async function uploadToImgur(imagePath) {
    ...
    const response = await axios.post('https://api.imgur.com/3/image', { image }, { headers: { Authorization: `Client-ID ${clientId}` } });
    return response.data.data.link;
}
export default class DescribeImageTool extends StructuredTool {
    constructor() {
        super();
        this.name = 'describe-image';
        this.description = '画像のパスを受け取り、その内容を文脈に沿って分析するツールです。画像の内容を理解する必要があるときに使用してください。';
        this.schema = z.object({
            image_path: z.string().describe('分析したい画像のパス'),
            context: z
                .string()
                .describe('分析する文脈。例は短いが実際はもっと詳細に指定すること。例:"ここはどのバイオームですか？", "指定した通りに建築できていますか？"'),
        });
    ...
    async _call(data) {
        ...
        try {
            ...
            return (response.choices[0].message.content || 'Failed to analyze the image.');
        }
        catch (error) {
